...
public class ReviewController {
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "수강후기 작성이 완료되었습니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "해당 학생이 존재하지 않습니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "40?", description = "해당 수강후기가 존재하지 않습니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "수강하지 않은 강의입니다. (reject 처리필요)")
    public ApiResponse writeReviewPost(@RequestBody ClassReviewRequest request
    ){
        reviewService.write(request);
        return ApiResponse.success(null, "수강후기 작성이 완료되었습니다.");
    }

    @DeleteMapping("/review")
    @Operation(summary = "수강 후기 삭제 요청", description = "수강 후기 삭제를 요청합니다. Long postId 만 body를 통해서 전송 {} 없이 요청하시면됩니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "수강후기 삭제가 완료되었습니다.")
    public ApiResponse deleteReviewPost(@RequestBody DeleteReviewRequest request
    ){
        reviewService.deleteReviewPost(request);
        return ApiResponse.success(null, "수강후기 삭제가 완료되었습니다.");
    }
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "수강후기 수정이 완료되었습니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "작성하지 않은 사용자의 요청 입니다.")
    public ApiResponse updateReviewPost(@RequestBody UpdateReviewRequest request
    ){
        reviewService.updateReviewPost(request); // 수정쪽 예외 한번더 검토해야함
        return ApiResponse.success(null, "수강후기 수정이 완료되었습니다.");
    }

    @PostMapping("/review/like")
    @Operation(summary = "수강 후기에 좋아요 요청", description = "수강 후기 좋아요를 요청합니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "좋아요가 추가되었습니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "202", description = "좋아요가 취소되었습니다.")
    public ApiResponse likeReviewPost(@Parameter(required = true, description = "int userNumber, Long postId를 json에 담아서 body로 전송") @RequestBody LikeRequest request
    ){
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",description = "해당 학생의 수강후기입니다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "202", description = "수강후기가 존재하지 않습니다.")
    public ApiResponse myReview(@RequestParam("userNumber") int userNumber){
        List<ReviewMeResponse> response = reviewService.findMyReview(userNumber);
        return ApiResponse.success(response, "해당 학생의 수강후기입니다.");
    }
